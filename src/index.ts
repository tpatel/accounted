...
createConnection().then(async connection => {
    ...
    console.log(await Account.authenticate('joe@example.com', 'insecurepassword'));
    console.log(await Account.authenticate('joe@example.com', 'false'));

    let success = await Account.updatePassword('joe@example.com', 'insecurepassword', 'insecure123456');
    console.log('Password update succeeded:', success);

    console.log(await Account.authenticate('joe@example.com', 'insecurepassword'));
    console.log(await Account.authenticate('joe@example.com', 'false'));

    const account2 = await Account.registerAccount('Jane', 'jane@example.com', 'insecurepassword');
